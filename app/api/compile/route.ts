...
// Fallback compilation for when Judge0 is not available
async function fallbackCompilation(language: string, code: string, input: string): Promise<CompileResponse> {
  // Simple pattern matching for common errors and basic execution simulation
  const commonErrors = {
    python: [
    ...
    java: [
      { pattern: /class.*expected/, message: 'Syntax Error: Missing class declaration' },
